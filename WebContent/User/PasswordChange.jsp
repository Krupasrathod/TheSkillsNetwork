...
<!--                 Reset Passord -->
                 ...
                 <div id="log-in" class="active">
                    ...
                    <form action="<%=request.getContextPath() %>/PasswordChangeController?flag=update" method="post">
                    
                     <c:forEach items="${sessionScope.UserNameotp}" var="i">
                      <input type="hidden" id="otpmail" name="otpmail" value="${i.email1}"> ${i.email1}
                     Resend OTP<a href="<%=request.getContextPath()%>/FpsswdController?flag=resendotp&ID=${i.userID}">Click Here</a>${i.userID}
                   
                     
                      </c:forEach>
                      <c:forEach items="${sessionScope.emailmatch}" var="j">
                   <%--   Forgot your password?<a href="<%=request.getContextPath()%>/FpsswdController?flag=resendotp&ID=${j.email}">Click Here</a>${j.userID}    --%>
                       <input type="hidden" name="userID" value="${j.userID}">
                       <input type="hidden" name="useremail" value="${j.email}">${j.userID}${j.email}
                     
                      </c:forEach>
                      <input name="otp" type="password" placeholder="OTP">
                      <input name="otppassword" type="password" placeholder="Password">
